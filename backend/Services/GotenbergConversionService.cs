...
public sealed class GotenbergConversionService
{
        ...
        IConfiguration configuration)
    {
        ...
        _gotenbergUrl = configuration["GotenbergUrl"] ?? "http://localhost:3000";
    }
    ...
    private async Task<byte[]> ConvertOfficeDocumentAsync(IFormFile file)
    {
        ...
        try
        {
            ...
            while (retryCount < MaxRetries)
            {
                try
                {
                    _logger.LogInformation($"Enviando arquivo {file.FileName} para Gotenberg em {_gotenbergUrl}{endpoint}");
                    var response = await _httpClient.PostAsync($"{_gotenbergUrl}{endpoint}", content);
